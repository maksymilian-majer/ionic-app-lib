...
State.restoreState = function restoreState(appDirectory, options) {
  ...
  var forcePluginInstall = !!options.force;

  return restorePromise
  .then(function() {
    if (options.plugins) {
      logging.logger.info('\rRestoring Plugins\n'.blue.bold)
      return State.restorePlugins(appDirectory, packageJson, forcePluginInstall)
      .then(function(){
...
State.restorePlugins = function restorePlugins(appDirectory, packageJson, force) {
  logging.logger.debug('State.restorePlugins', appDirectory, packageJson, force);
  var q = Q.defer();
  State.processPlugin(appDirectory, 0, packageJson, force, q);
  return q.promise;
};
...
State.createAddRemoveStatement = function createAddRemoveStatement(plugin, force) {
    ...
    try {
      ...
      // when the --force parameter is passed, then the plugin will install
      // even if the sources already exist in the target directory
      if (force) {
        pluginCmd += ' --force';
      }
    } catch (ex) {
...
State.processPlugin = function processPlugin(appDirectory, index, packageJson, force, promise) {
  ...
  try {
    ...
    var pluginCommand = State.createAddRemoveStatement(plugin, force);
    ...
    shelljs.exec(pluginCommand, {async: true}, function(code, output) {
      ...
      State.processPlugin(appDirectory, index + 1, packageJson, force, promise)
    });
  } catch (ex) {
    logging.logger.error('An error happened processing the previous cordova plugins')
    logging.logger.error(ex)
...
State.resetState = function resetState(appDirectory, options) {
  ...
  var removeMessage = 'Removed ';

  if (options.platforms) {
    paths.push(platformPath);
    removeMessage += 'platforms';
  }
  if (options.plugins) {
    paths.push(pluginPath);
    removeMessage += options.platforms ? ' and plugins' : ' plugins';
  }

  shelljs.rm('-rf', paths);
    logging.logger.info(removeMessage.blue.bold);
  return State.restoreState(appDirectory, options)
    .then(function() {
